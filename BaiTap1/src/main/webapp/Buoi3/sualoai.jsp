...
<body
	...
	<div class="w-full max-w-lg">
		...
		<form method="post" action="/SuaLoai"
			class="bg-white p-8 rounded-xl shadow-2xl space-y-6">

			<h2 class="text-3xl font-bold text-gray-900 text-center mb-6">Cập
				Nhật Loại Sách</h2>
			...
			<div class="space-y-2">
				<label for="maLoaiHienThi"
					class="block text-sm font-medium text-gray-700">Mã Loại
					(Khóa chính):</label> <input type="text" id="maLoaiHienThi"
					value="${loai.maLoai}" readonly
					class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-600 font-semibold cursor-not-allowed">
			...
			<div class="space-y-2">
				<label for="tenLoai" class="block text-sm font-medium text-gray-700">Tên
					...
					value="${param.tenLoai != null ? param.tenLoai : loai.tenLoai}"
					...
					placeholder="Nhập tên loại mới" required>
			</div>

			<input type="submit" name="btnsua" value="Cập Nhật Loại"
